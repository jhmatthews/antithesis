...
equations~\ref{eq:planck}, \ref{eq:boltzmann}
and \ref{eq:jbar2}  can be combined to give
\begin{equation}
\frac{2 h \nu_{ul}^3}{c^2} \frac{1}{\exp(h\nu_{ul} / kT) - 1} =
\frac{A_{ul}}{B_{ul}} \frac{1}{(g_l/g_u)(B_{lu}/B_{ul}) \exp(h\nu_{ul} / kT) - 1}.
\end{equation}
...
The code can operate with a numnber of different coordinate systems 
...
the wind are then to Saha (LTE) abundances at $T_{\mathrm{init}}$, and the level 
...
\end{equation}
where, under the Eddington approximation used in the code, $\eta(\theta)$
is given by
\begin{equation}
...
\frac{(1 - e^{-h\nu_{ju}/kT_e})}{(e^{h\nu_{ju}/kT_e} - 1)}  h \nu_{ju}.
\end{equation}
These estimators are fundamentally different quantities to the corresponding
