...
term. Inspection of equations~?? and ?? reveals that a negative
photoionization rate would be obtained in this situation.

To prevent this problem, we can simply `clean' our level populations
after calculation, as suggested by \cite{lucy2002}. 
...
Ar, Ca and Fe, although this can be easily be adapted. Generally, solar abundances from
\cite{vernerbarthel1994} are adopted, although for supernova models these 
are modified to reflect the higher metallicity. Ionization potentials and ion
multiplicities are from \cite{verner1996}. Line information for simple-atoms 
is obtained from \citep{verner1996} ($\sim5,000$ lines) and \citep{kurucz1994} 
...
singlet and triplet lines observed in many optical spectra. He~\textsc{ii} assumed
to be $l$-mixed as it is a hydrogenic ion.
...
This can provide a performance hit in macro-atoms (see section~??), but is necessary
as we were concerned with modelling recombination lines from upper levels
close to the continuum energy. In quasar models (chapter 5) 
this was not as important, and the
...
on figure~??, and the effect on quasar models is discussed in chapter 5. 
