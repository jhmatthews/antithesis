...
During the ionization cycles, the energy absorbed into $k$-packets and 
every macro-atom level is recorded using the following estimators. Once 
...
Once the emissivites have been calculated then the spectral synthesis can proceed.
...
$k$-packer emissivities in each cell. To now ensure that radiative equilibrium 
...
against \cld\ in the past; some of these tests can be found in H13 and LK02,
and generally show excellent agreement, especially in lower $Z$ ions.

Figs.~\ref{fig:h_cloudy} to \ref{{fig:he_cloudy}} show a series of 
ionization plots comparing relative ion fractions as a function of
ionization parameter, $U$. I have shown two separate ionization modes
in \py: standard mode, in which nothing is treated as a macro-atom
and the spectral modelling ionization scheme is used to calculate
...
described in section~\ref{sec:matom_pops}.
...
emissivity calculation described above, as opertions on cells or photons can
