...
\subsection{Local thermodynamic equlibrium}
...
When the assumption of LTE is made three {\em microscopic} 
requirements also follow \citep{mihalas}:
...
when high-energy photons create suprathermal electron distributions (REFs). 
...
\begin{equation}
 \dot{p}_{j}^{R} = \dot{E}_{j}^{R} / D_j \;\;\;\;\; {\rm and} \;\;\;
...
are given by
\begin{equation}
 \dot{p}_{j\up} = {\cal R}_{ju} \epsilon_{j} / D_j \;\;\;\;\; {\rm and} \;\;\;
\;\; \dot{p}_{j\ellp} = {\cal R}_{j\ellp} \epsilon_{\ellp} / D_j.
\end{equation}
...
\subsection{Macro-atom Line Transfer}
