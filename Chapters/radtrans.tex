...
\label{eq:vanregemorterw}
\end{equation}
where $\sigma_{\nu_0}$ is the photoionziation cross-section at the threshold energy.
...
\begin{figure}
\centering
\includegraphics[width=1.0\textwidth, clip=true, trim=0 1.5in 0in 0in ]{figures/03-radtrans/flow_colour.pdf}
\caption
{
...
This is done via matrix inversion. For an element with $n$ ions and $m_n$
levels in each ions, we construct a matrix with dimensions $\sum_n m_n$.
This matrix is then populated with the rates 
...
inverting large matrices and encorporates error handling and enables
