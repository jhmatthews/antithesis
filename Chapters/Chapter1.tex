...
\index{accretion}
centres of many galaxies -- accreting supermassive black holes\index{black hole!supermassive} 
...
\index{quasar}\index{cataclysmic variable}\index{neutron star}\index{white dwarf}
\index{accretion}\index{nuclear fusion}\index{AGN}\index{accretion disc}
...
and there is even evidence of radio emission in\index{outflows}\index{radio jet}
CVs \citep{benz1983,kordingDNjet2008,coppejans2015}. 
These radio jets tend to appear in specific 
accretion states \citep{fender2001,fender2004,kordingDNjet2008},\index{accretion!state}
implying an intrinsic connection to the 
accretion process. Even more intriguing, less collimated, mass-loaded outflows\index{outflows}
or {\em winds} are observed in XRBs in soft accretion states, 
possibly emanating from the accretion disc \citep{ponti2012}.
Evidence for disc winds is widespread across the mass range,\index{disc wind} 
...
\index{BALs}\index{absorption line}\index{BALQSOs}
seen in high-state CVs \citep{heap1978,greensteinoke1982,cordova1982}\index{cataclysmic variable}
...
\index{ultra-fast outflow}\index{BALQSOs}\index{BALs}\index{stellar winds}\index{iron line}
...
via a `feedback' mechanism \citep{king2003,fabian2012}.\index{AGN!feedback} 
...
\index{quasar}\index{AGN}\index{radio jet}\index{X-ray binary}\index{AGN!unification}
\index{outflows}\index{disc wind}
...
the Monte Carlo radiative transfer and photoionization\index{Monte Carlo radiative transfer}
...
The basic phenomenon of accretion --\index{accretion} 
...
object is given by\index{accretion!power} 
\begin{equation}
...
\label{eq:restmass2}
\end{equation} 
Nuclear fusion\index{nuclear fusion}  is one of the more efficient
...
a characteristic scale for any compact object of mass $M$.\index{event horizon} 
...
symmetric manner.\index{accretion!spherical} 
...
accretion onto a point mass and derived the Bondi radius,\index{Bondi-Hoyle-Lyttleton accretion}
\begin{equation}
r_B = \frac{G M}{c_S^2},
\label{eq:bondi}
\end{equation} 
...
For a cloud of uniform density, $\rho_0$, this is given by\index{free-fall timescale}
\begin{equation}
...
radiatively inefficient accretion flows (RIAFs).\index{ADAF}\index{RIAF}
...
when we consider that molecular viscosity\index{viscosity!molecular}
...
candidate is the magnetorotational instability \citep[MRI; ][]{balbus1991},\index{MRI}
...
an {\em effective viscosity}\index{viscosity}.
...
\index{accretion disc!thin disc models}
...
of the viscosity, $\nu^\prime$, using the simple form\index{viscosity!alpha parameter}
\begin{equation}
\nu^\prime = \alpha c_s H,
\label{eq:viscosity}
\end{equation}
...
the mass accretion rate so as to achieve a steady state.\index{viscosity}
...
a compact object rotating with angular velocity $\Omega_*$. The boundary layer (BL)\index{boundary layer}
...
\index{innermost stable circular orbit}
...
is shown for different values of the BH spin parameter, $a_*$,\index{black hole spin} 
...
\index{black hole!Kerr}\index{black hole!Schwarzschild}
...
follows Planck's law:\index{blackbody}
\begin{equation}
...
and its companion as the `secondary'.\index{X-ray binary!high mass} 
...
\index{compact binary}

\subsection{Roche Lobe-Overflow}
\label{sec:rlof}
\index{Roche Lobe overflow} 
...
\index{Roche potential}
In the context of semi-detached binary systems, the most important equipotential
...
\citep[see the `CV bible', ][]{warnerbook}.\index{cataclysmic variable}\index{white dwarf} 
CVs are not always dominated by their accretion luminosity; 
for example, novae undergo dramatic eruptions caused\index{supersoft source}
...
is dominated by the WDs magnetic field.\index{polar CV} 
\index{intermediate polar} 
...
most famous DNe is SS Cyg, whose light curve is shown in \index{SS Cyg}
...
section.\index{accretion disc}

The leading scenario for explaining DN outbursts, and in fact also the outbursts
in low mass X-ray binaries or `soft X-ray transients',\index{viscosity}
...
Nova-like variables (NLs) are similar to DNe,\index{nova-like variable} 
...
cases, and particularly in the SW~Sex subclass of NLs\index{SW Sex} 
...
exhibit such double-peaked lines \citep{marshhorne1990}.\index{emission line} 
...
Although I do not study low-mass X-ray binaries (LMXBs)\index{X-ray binary!low mass} 
...
The accretion disc \index{neutron star}\index{black hole}\index{X-ray! power law}
...
to Compton up-scattering of seed disc photons by some kind of `corona'\index{Compton scattering}
of hot electrons close to the BH \citep[e.g.][]{white1988,mitsuda1989,uttley2014}.
The discovery that XRBs and CVs follow similar \index{X-ray! corona}
...
\index{AGN}\index{quasar}\index{Seyfert galaxy}   
...
If these broad lines are seen, then the AGN is classed as type 1;\index{AGN!type 1}\index{AGN!type 2}
...
unusual types of AGN: narrow line Seyfert 1s (NLSIs), which\index{narrow line Seyfert 1}
...
\citep{tran2001,shi2010} rather than obscured (see next section).\index{AGN!true type 2}
...
Although Seyfert had identified type 1 and 2 AGN, a physical explanation\index{AGN!unification}
...
to unify AGN behaviour, as it elegantly \index{BLR} 
...
are formed in the narrow-line region (NLR), which is on a\index{NLR}  
significantly larger scale and is thus always visible.
The obscuring structure became known as the `torus' \citep{krolik1986},\index{torus}  
...
AGN \citep{matt2003,puccetti2007},\index{AGN!changing-look}
including even NGC 1068 itself \citep{marinucci2016}.\index{NGC 1068}
...
for this switch \citep{elitzur2014}.\index{disc wind}\index{accretion state}
\index{emission lines} Furthermore, dusty {\em polar} outflows
...
that can be tested with observations.\index{torus} 
...
widely considered, as in LMXBs, to come from a hot `corona' of\index{X-ray! corona} 
electrons close to the BH that upscatters disc seed photons\index{Compton scattering} 
...
of high energy photons \citep{pounds1989,nandra1994}.\index{Compton hump}\index{Compton scattering}
...
If this is the case, and the broadening of the iron line is relativistic,\index{iron line}
this would allow for measurements of the BH spin 
\citep{laor1991,iwasawa1996a,dabrowski1997}.\index{black hole spin}
...
broad red Fe K$\alpha$ wing \citep{sim2010}.\index{iron line}

\index{AGN!Compton-thick} 
...
This is known as the soft X-ray excess (SXXS), which is too\index{soft X-ray excess} 
...
explain this excess, including relativistically smeared \index{general relativity}
photoabsorption \citep{gierlinskidone2004b,gierlinskidone2006}, 
relativistically smeared line and \index{free-free emission}\index{Compton scattering}
...
\index{BLR}\index{AGN!unification}\index{emission line}\index{disc wind}
...
it has been suggested that orientation is a key driver of the so-called\index{Eigenvector 1}
...
\index{Occam's razor}\index{Occam's quasar}\index{disc wind}
...
its applicability to accreting systems.\index{accretion disc} 
\index{accretion disc!thin disc models} 
...
the UV spectrum.\index{Balmer edge}\index{recombination continuum} 
\index{stellar atmosphere} 
...
\index{accretion disc!disc-size problem} 
...
% is that we do observe a slope in the UV of $\alpha_{UV} \approx 0.3$, confirming
...
abandon the good ship ``Shakura-Sunyaev'' just yet.\index{accretion disc}
...
\index{accretion disc!fluctuating}
...
low states \citep{gu2009,dimitri_llagn,connolly2016}.\index{fundamental plane of BH activity}
...
\index{radio jet}\index{black hole spin}\index{RIAF}
