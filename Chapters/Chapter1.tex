...
X-ray emission, but also due to changes in the absorption characteristics (REFs).
I discuss X-ray absorption in more detail, particular with respect to winds, in section~?.
...
It is still unclear where these features originate, but it is often proposed that
they are caused by reflection off the inner parts of an accretion disc (REFs).
This model may help explain some of the variability of AGN (REFs), and modelling
of the relativistic iron line allows one to make BH spin measurements (REFs).
In Compton-thick AGN, the intrinsic continuum is heaviliy absorbed with columns of
$N_H\sim10^{24}$cm$^{-2}$ -- this absorption is normally attributed to the dusty torus (REF), 
but Compton-thick winds could also contribute. 
In these sources, reflection features can actually dominate the X-ray spectrum (REFs).
...
However, AGN spectra do not generally exhibit the spectral shapee expected
...
then they follow very similar tracks \citep{kordingDNjet2008}.
...
(REFs), but whether such a correlation exists in LMXBs is controversial (REFs).
...
radio emission (REFs). This important result suggests that our understanding
of jets is incomplete, and that the links between accretion state and 
jet production are fundamental, but unsolved. Disc winds may in fact complicate 
matters further (see chapter 2).
