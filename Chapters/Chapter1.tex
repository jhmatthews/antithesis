...
for four different compact objects are shown in table~\ref{compactness}. 
...
\citep{pringle1991}. The reasons for this are readily apparent
when we consider that molecular viscosity operates over the Debye length,
$\lambda_D$, and at the sound speed, $c_S$. The viscous force density
is therefore much less than the inertial force density (or in other words
the Reynolds number is much greater than 1); the consequence being that
...
that the angular momentum is lost via a a magnetohydrodynamic 
outflow \citep{blandfordpayne} or spiral shock waves
\citep{ju2016}, although the former cannot be the main angular momentum loss
mechanism if the disc is to remain intact \citep[e.g.][]{knigge}. 
Given the uncertainty about the angular momentum transport mechanism, it is 
