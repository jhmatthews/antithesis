...
\civfull\ \citep[][see Fig.~\ref{fig:cordova}]{cordova1982}). 
...
of line photons \citep[see also Fig.~\ref{fig:mc_line} and][]{flohic2012}. 
...
as described. The effect of an accretion disc wind
...
of $\sim1500$km~s$^{-1}$. A series of papers also found 
X-ray absorption features in similar objects \citep{ueda1998,kotani2000,parmar2002}. 
These absorption features tended to be detected
in high-inclination, `dipping' LMXBs. This was confirmed in more sources
by \cite{ponti2012}, who proposed an equatorial geometry based on this (see 
Fig.~\ref{fig:ponti_cartoon}). 
The same study demonstrated (Fig.~\ref{fig:ponti_hid}) that
...
Fig.~\ref{fig:bals}. In addition to the most common high-ionization
...
alter its true value. The degree of collimation of the BAL wind
...
to object, as shown in Fig.~\ref{fig:bals}. The 
...
Fig.~\ref{fig:nardini} with a simply spherical outflow model fit, 
...
transfer code in chapter 5, before presenting
...
produces good matches with observations \citep[e.g.][]{pauldrach1994,hamann2008}. 
...
In alternative magnetic models the isotropic
magnetic pressure ($B/8\pi$ in CGS units)
is responsible for driving the outflow (REFs).
In this case the toroidal component 
is responsible for this pressure and drives an outflow which
behaves more like a thermally-driven wind 
(i.e. conserves angular momentum rather than angular velocity)
There is still great uncertainty over the magnetic fields in accretion discs
and the physics of these magnetic processes,
but in many senses they are attractive mechanisms as magnetic processes
are already expected to be important in accretion disc winds
due to the MRI.
...
A diagram of the geometry is shown in Fig.~\ref{fig:MCGV95}.
...
important for other reasons. In a subsequent study 
\citep[][hereafter H14]{H14} showed that in this particular geometry 
...
radii $r_{min}$ and $r_{max}$. A schematic is shown in Fig.~\ref{fig:sv_cartoon}.
...
\label{eq:rho0}
\end{equation}
and at a coordinate $(r,z)$ the density will be 
\begin{equation}
...
passes through $(r,z)$. I have now specified the equations that govern the kinematics
and densities in the wind for the SV93 prescription. 
This prescription is used to describe the outflow in
the radiative transfer code \py. The radiative transfer procedure and 
...
$10^{15}$~cm across, a billionth of the radius of a typical galactic bulge. This is 
...
$\sigma_* = 0.001c$ and $M_{BH}^\prime / M_{bulge} = 10^{-3}$ we can show that 
\begin{equation}
...
various numerical simulations \citep[e.g.][]{springel2005,croton2006}.

There is also more direct evidence that AGN are at least energetically 
significant when compared to the galactic bulge. X-ray observations
of cool core clusters and elliptical galaxies
can show dramatic X-ray cavities or bubbles on scales of
up to 50~kpc, with a radio-loud AGN at the centre
...
This model naturally reproduced the observed slope of the $M_{BH}-\sigma_*$
...
for example, major galaxy mergers may explain the colour `red and dead' branch
of the galaxy colour bimodality \citep[e.g.][]{somerville2001,baldry2004}. 
Regardless of the effect of mergers, AGN winds and jets are clearly 
...
Now the astrophysical importance of outflows has 
been established, I shall move on to discussing how we might go about 
accurately modelling the ionization states and emergent spectra from
systems with accretion disc winds.
