...
with a high EW tail of slope $-3.5$. We can now vary the maximum angle, $\theta_1$,
...
seleciton effects and lack of knowledge about the true face-on
distribution.
...
$\theta_{1}=84^\circ$ and a slightly broader intrinsic distribution
than model 1 of R11. I adopt linear binning
to facilitate easy comparison with R11, and only fit up $EW=100$\AA
...
\subsection{Comparing non-BAL and BAL Distributions}
...
non-BALs I repeat the process described in section~\ref{sec:fitting} with a few 
key differences
...
% \subsection{Fitting the Quasar EW Distribution}

% It is possible to fit the EW distribution of quasars for a given
% form of \ept\ if one knows either the distribution of viewing angles 
% or the intrinsic `face-on' EW distribution of the emission line in 
% question. Unfortunately, neither of these distributions are known and deconvolving
% their physical effects is challenging. If an intrinsic, face-on distribution could be constructed
% then the models of continuum emission introduced above 
% could be used to derive predicted EW distributions
...
% e.g., a Kologomorov-Smirnov test. 
